...
\begin{abstract}
\noindent
A busca por novos materiais supercondutores, especialmente aqueles que exibem o fenômeno a temperaturas mais elevadas, é uma das fronteiras mais ativas da física da matéria condensada. A complexidade dos mecanismos subjacentes, particularmente em supercondutores não convencionais, torna a predição teórica a partir de primeiros princípios uma tarefa computacionalmente custosa e, por vezes, intratável. Este trabalho explora uma abordagem alternativa, fundamentada na física computacional e na ciência de dados, para classificar materiais como supercondutores ou não-supercondutores com base em um conjunto de descritores físicos. Utilizando um banco de dados público contendo mais de 2.400 compostos, foram implementados e avaliados seis modelos de aprendizado de máquina: Random Forest, XGBoost, LightGBM, Support Vector Machine (SVM), Multilayer Perceptron (MLP) e LASSO. A metodologia envolveu um rigoroso pré-processamento dos dados, treinamento dos modelos e uma análise comparativa detalhada utilizando métricas como F1-Score, Acurácia e ROC-AUC. Os resultados demonstram a viabilidade da abordagem, com o modelo Random Forest alcançando o melhor desempenho, com um F1-Score de 0.723 e uma área sob a curva ROC de 0.651 no conjunto de teste. Uma análise da importância das \textit{features} e de Partial Dependence Plots revelou que propriedades da densidade de estados eletrônicos (DOS) e da estrutura de bandas no nível de Fermi são os descritores mais preditivos, corroborando a intuição física de que a supercondutividade está intrinsecamente ligada ao comportamento dos elétrons na superfície de Fermi. Uma análise de erro detalhada mostrou que os modelos têm maior dificuldade em classificar supercondutores convencionais de baixa temperatura crítica ($T_c < 5$ K). Este estudo não apenas fornece um modelo preditivo funcional, mas também reforça a sinergia entre o aprendizado de máquina e a física, abrindo caminhos para a triagem acelerada de novos materiais promissores.
\vspace{\baselineskip}
...
\begin{abstract}
\noindent
The search for new superconducting materials, especially those exhibiting the phenomenon at higher temperatures, is one of the most active frontiers in condensed matter physics. The complexity of the underlying mechanisms, particularly in unconventional superconductors, makes theoretical prediction from first principles a computationally expensive and sometimes intractable task. This work explores an alternative approach, grounded in computational physics and data science, to classify materials as superconductors or non-superconductors based on a set of physical descriptors. Using a public database containing over 2,400 compounds, six machine learning models were implemented and evaluated: Random Forest, XGBoost, LightGBM, Support Vector Machine (SVM), Multilayer Perceptron (MLP), and LASSO. The methodology involved rigorous data preprocessing, model training, and a detailed comparative analysis using metrics such as F1-Score, Accuracy, and ROC-AUC. The results demonstrate the feasibility of the approach, with the Random Forest model achieving the best performance, with an F1-Score of 0.723 and an area under the ROC curve of 0.651 on the test set. An analysis of feature importance and Partial Dependence Plots revealed that properties of the electronic density of states (DOS) and band structure at the Fermi level are the most predictive descriptors, corroborating the physical intuition that superconductivity is intrinsically linked to the behavior of electrons at the Fermi surface. A detailed error analysis showed that the models have greater difficulty classifying conventional superconductors with low critical temperature ($T_c < 5$ K). This study not only provides a functional predictive model but also reinforces the synergy between machine learning and physics, paving the way for the accelerated screening of promising new materials.
\vspace{\baselineskip}
...
A descoberta de novos supercondutores, especialmente aqueles com altas temperaturas críticas, tem implicações tecnológic =============================================================================
% CAPÍTULO 2: REVISÃO TEÓRICA - FÍSICA DA SUPERCONDUTIVIDADE
% =============================================================================
\chapter{Revisão Teórica: Física da Supercondutividade}
\label{chap:fisica}

Neste capítulo, apresentamos os conceitos fundamentais da física da supercondutividade, fornecendo a base teórica necessária para a compreensão do problema de classificação e a interpretação dos resultados obtidos com os modelos de aprendizado de máquina.
...
\begin{itemize}
    ...
    \item O tamanho típico de um par de Cooper, dado pelo \textbf{comprimento de coerência} $\xi_0$, é da ordem de centenas a milhares de angstroms, muito maior que a distância interatômica. Isso significa que os pares de Cooper se sobrepõem fortemente, formando um condensado altamente correlacionado.
\end{itemize}
...
Esta equação é fundamental. Ela mostra que, mesmo para $\epsilon_k = 0$ (elétrons exatamente no nível de Fermi), a energia de excitação mínima é $\Delta$, o gap supercondutor. Não existem excitações de baixa energia no estado supercondutor, o que explica a ausência de resistência elétrica: não há estados disponíveis para os elétrons espalharem.
...
onde $F_n$ é a energia livre do estado normal, $\alpha$ e $\beta$ são coeficientes fenomenológicos (com $\alpha$ mudando de sinal em $T_c$), $m^* = 2m$ e $e^* = 2e$ são a massa e a carga efetivas do par de Cooper, e $\vec{A}$ é o potencial vetor magnético.
...
\begin{itemize}
    ...
    \item \textbf{Profundidade de Penetração GL, $\lambda(T)$:} A escala de distância sobre a qual o campo magnético decai no supercondutor. É dado por $\lambda(T) = \sqrt{m^* c^2 \beta / (4\pi (e^*)^2 |\alpha|)}$.
\end{itemize}
...
Uma das descobertas mais empolgantes das últimas décadas foi a observação de supercondutividade em hidretos sob pressões extremas. Em 2015, Drozdov et al. relataram supercondutividade em H$_3$S a 203 K sob uma pressão de 150 GPa, quebrando o recorde de $T_c$. Mais recentemente, em 2020, foi reportada supercondutividade em LaH$_{10}$ a aproximadamente 250 K (cerca de $-23$ °C) sob 170 GPa, aproximando-se da temperatura ambiente.
...
O dataset final (\texttt{data\_treino.csv}) contém 2.474 amostras de materiais, cada uma descrita por 130 \textit{features} (descritores) e uma variável alvo, a temperatura crítica ($T_c$).

As \textit{features} podem ser agrupadas em categorias:
\begin{itemize}
    \item \textbf{Propriedades Atômicas:} Informações sobre os elementos constituintes, como número atômico médio, eletronegatividade média, raio atômico médio, massa atômica média, etc. Essas propriedades são calculadas como estatísticas (média, desvio padrão, etc.) sobre os átomos na fórmula química.
    \item \textbf{Propriedades Eletrônicas:} Descritores derivados de cálculos de estrutura de bandas, como a densidade de estados (DOS) no nível de Fermi, a energia média das bandas, e características da superfície de Fermi.
    \item \textbf{Propriedades Termodinâmicas:} Informações como a temperatura de Debye, relacionada às propriedades vibracionais da rede.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Definição do Problema de Classificação:} A variável alvo contínua, $T_c$, foi convertida em uma variável binária. Materiais com $T_c > 0$ K foram rotulados como classe 1 (supercondutor), e materiais com $T_c = 0$ K ou valores ausentes foram rotulados como classe 0 (não-supercondutor). Isso resultou em um conjunto de dados com 1362 supercondutores (55.1\%) e 1112 não-supercondutores (44.9\%), um conjunto relativamente balanceado.
    
    \item \textbf{Limpeza de Dados:} Colunas não-numéricas e identificadores (como \texttt{group\_id} e \texttt{id}) foram removidos. Colunas com mais de 50\% de valores ausentes também foram descartadas para evitar a introdução de ruído excessivo.
    ...
    \item \textbf{Escalonamento de Features:} Para os modelos sensíveis à escala das \textit{features} (SVM, MLP, LASSO), foi aplicado o \texttt{StandardScaler}, que transforma cada \textit{feature} para ter média 0 e desvio padrão 1: $x' = (x - \mu) / \sigma$.
\end{enumerate}

Após o pré-processamento, o número de \textit{features} utilizadas para o treinamento foi reduzido para 54.
...
A escolha dos hiperparâmetros de cada modelo é crucial para o desempenho e a generalização. Neste trabalho, os hiperparâmetros foram definidos com base em valores padrão bem estabelecidos na literatura, sem uma busca exaustiva (\textit{grid search} ouA escolha dos hiperparâmetros de cada modelo é crucial para o desempenho e a generalização. Neste trabalho, os hiperparâmetros foram definidos com base em valores padrão bem estabelecidos na literatura, sem uma busca exaustiva (\textit{grid search}). Essa escolha foi motivada pelo tamanho modesto do dataset, onde uma busca exaustiva poderia levar a \textit{overfitting}, e pelo foco do trabalho na interpretabilidade física em detrimento da otimização máxima do desempenho. Para mitigar o risco de \textit{overfitting}, foram utilizadas estratégias como validação cruzada, regularização e limitação da complexidade dos modelos. =============================================================================
...
\begin{table}[H]
    \centering
    \caption{Resultados de desempenho dos modelos no conjunto de teste.}
    \label{tab:resultados}
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Modelo} & \textbf{Acurácia} & \textbf{Precisão} & \textbf{Recall} & \textbf{F1-Score} & \textbf{ROC-AUC} \\
        \midrule
        Random Forest & 0.6727 & 0.6729 & 0.7912 & \textbf{0.7233} & \textbf{0.6514} \\
        XGBoost & 0.6606 & 0.6744 & 0.7436 & 0.7073 & 0.7230 \\
        LightGBM & 0.6566 & 0.6700 & 0.7436 & 0.7049 & 0.7200 \\
        SVM (RBF) & 0.6444 & 0.6655 & 0.7143 & 0.6890 & 0.6670 \\
        MLP & 0.6081 & 0.6100 & 0.8022 & 0.6930 & 0.6400 \\
        LASSO (L1) & 0.5657 & 0.5843 & 0.7363 & 0.6515 & 0.5936 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{graficos/04_comparacao_metricas.png}
...
Os modelos baseados em árvores de decisão (Random Forest, XGBoost, LightGBM) apresentaram o melhor desempenho geral. O \textbf{Random Forest} foi o campeão, com o maior F1-Score (0.7233) e ROC-AUC (0.6514). Isso sugere que a capacidade do Random Forest de reduzir a variância através do \textit{bagging} e da aleatorização de \textit{features} é particularmente eficaz para este tipo de dados, que pode conter ruído e relações não-lineares complexas.

O MLP, apesar de seu alto recall (0.8022), sofreu de baixa precisão (0.6100), indicando uma tendência a classificar materiais como supercondutores de forma mais liberal, gerando muitos falsos positivos. O modelo LASSO teve o desempenho mais fraco, com um ROC-AUC de apenas 0.5936, sugerindo que um modelo linear simples, mesmo com regularização, não é suficiente para capturar a complexa relação entre os descritores e a supercondutividade.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\linewidth]{graficos/05_curvas_roc.png}
        \caption{Curvas ROC}
        \label{fig:roc}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\linewidth]{graficos/06_curvas_precision_recall.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/09_importancia_features.png}
...
De acordo com a teoria BCS (Equação 2.10), a temperatura crítica $T_c$ é exponencialmente dependente da densidade de estados no nível de Fermi, $N(0)$. Uma alta densidade de estados significa que há mais elétrons disponíveis para formar pares de Cooper, favorecendo o estado supercondutor. O fato de os modelos de aprendizado de máquina, sem nenhum conhecimento prévio da teoria BCS, terem identificado a DOS como o fator mais preditivo é uma forte validação da abordagem e demonstra a capacidade do ML de extrair conhecimento físico a partir de dados.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/19_analise_erro.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{graficos/11_validacao_cruzada.png}
...
Para complementar a análise de classificação, o modelo SISSO foi aplicado à tarefa de regressão, com o objetivo de encontrar uma expressão analítica para a temperatura crítica $T_c$ em função dos descritores físicos. A análise foi realizada apenas no subconjunto de materiais supercondutores ($T_c > 0$).

O desempenho do modelo, embora modesto (R² de 0.3537 no conjunto de teste), é secundário em relação à interpretabilidade da equação descoberta. O SISSO encontrou a seguinte expressão com 5 termos:

\begin{equation}
    T_c \approx 3.41 - 3.32 \cdot D_{min} - 3.10 \cdot D_{min} \cdot p_{0,2} + 3.02 \cdot D_{min} \cdot c_{0,1} + 2.71 \cdot c_{0,1} \cdot c_{2,2} - 2.60 \cdot D_{min} \cdot p_{1,2}
\end{equation}

onde $D_{min}$ é a densidade de estados mínima, $p_{i,j}$ são componentes do vetor de posição e $c_{i,j}$ são componentes da célula unitária.
...
O resultado mais importante é a proeminência da densidade de estados ($D_{min}$) como a \textit{feature} mais influente, aparecendo em 4 dos 5 termos da equação. Isso corrobora a teoria BCS, que prevê uma dependência exponencial de $T_c$ com a densidade de estados no nível de Fermi. O SISSO, de forma autônoma, "redescobriu" a importância central deste descritor físico.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{graficos/08_ranking_modelos.png}
...
O modelo Random Forest emergiu como o mais eficaz, alcançando um F1-Score de 0.723 e um ROC-AUC de 0.651 no conjunto de teste. Mais importante do que o desempenho preditivo em si foi a análise da interpretabilidade do modelo. A identificação da densidade de estados eletrônicos (DOS) e das características da superfície de Fermi como os descritores mais importantes estabelece uma ponte crucial entre a ciência de dados e a física da matéria condensada. Esse resultado não apenas corrobora décadas de teoria e experimentação, mas também ilustra o potencial do aprendizado de máquina como uma ferramenta para a descoberta científica, capaz de redescobrir princípios físicos a partir de dados brutos.
...
\begin{enumerate}
    ...
    \item \textbf{Vieses no Dataset:} O banco de dados SuperCon é composto principalmente por materiais que foram estudados experimentalmente, o que pode introduzir vieses. Materiais que nunca foram sintetizados ou testados não estão representados.
...
\begin{itemize}
    ...
    \item \texttt{scikit-learn} (v1.x): Implementação dos modelos Random Forest, SVM, MLP e LASSO, além de ferramentas de pré-processamento e avaliação.
...
\begin{table}[H]
    ...
    \begin{tabular}{llll}
        ...
        \midrule
        Stanev et al. (2018) & Regressão de $T_c$ & Gradient Boosting & $R^2 \approx 0.92$ \\
        Hamidieh (2018) & Regressão de $T_c$ & XGBoost & RMSE $\approx 9.5$ K \\
        \textbf{Este trabalho} & Classificação & Random Forest & F1-Score = 0.723 \\
        \bottomrule
    \end{tabular}
\end{table}
...
Este apêndice fornece uma descrição detalhada das principais \textit{features} (descritores) utilizadas neste trabalho, conectando-as aos conceitos físicos relevantes.

\section{Features Relacionadas à Densidade de Estados (DOS)}

A densidade de estados eletrônicos, $N(E)$, é uma função que descreve o número de estados eletrônicos disponíveis em um determinado intervalo de energia. No contexto da supercondutividade, a DOS no nível de Fermi, $N(E_F)$, é particularmente importante, pois determina o número de elétrons disponíveis para formar pares de Cooper.

\begin{table}[H]
    \centering
    \caption{Descrição das \textit{features} relacionadas à densidade de estados.}
    \begin{tabular}{lp{10cm}}
        \toprule
        \textbf{Feature} & \textbf{Descrição Física} \\
        \midrule
        \texttt{sc\_DOSs\_mean} & Média da densidade de estados na região próxima ao nível de Fermi. Valores altos indicam mais elétrons disponíveis para o pareamento. \\
        \texttt{sc\_DOSs\_min} & Valor mínimo da DOS na região de interesse. Valores muito negativos podem indicar a presença de gaps ou pseudogaps. \\
        \texttt{sc\_DOSs\_max} & Valor máximo da DOS. Picos na DOS podem indicar singularidades de Van Hove, que são associadas a altas $T_c$ em alguns materiais. \\
        \texttt{sc\_DOSs\_std} & Desvio padrão da DOS. Uma DOS mais uniforme (baixo desvio padrão) pode ser característica de certos tipos de supercondutores. \\
        \bottomrule
    \end{tabular}
\end{table}

\section{Features Relacionadas à Estrutura de Bandas}

A estrutura de bandas descreve a relação entre a energia dos elétrons e seu momento cristalino. As bandas que cruzam o nível de Fermi são particularmente importantes, pois determinam as propriedades de transporte e a superfície de Fermi.

\begin{table}[H]
    \centering
    \caption{Descrição das \textit{features} relacionadas à estrutura de bandas.}
    \begin{tabular}{lp{10cm}}
        \toprule
        \textbf{Feature} & \textbf{Descrição Física} \\
        \midrule
        \texttt{sc\_bands\_mean} & Energia média das bandas na região do nível de Fermi. Relacionada à posição das bandas em relação ao potencial químico. \\
        \texttt{sc\_bands\_std} & Desvio padrão das energias das bandas. Uma alta dispersão pode indicar uma estrutura de bandas complexa. \\
        \texttt{sc\_bands\_min} & Energia mínima das bandas. Pode indicar a presença de bandas profundas que não contribuem diretamente para a supercondutividade. \\
        \texttt{sc\_bands\_max} & Energia máxima das bandas. Bandas de alta energia podem indicar estados anti-ligantes. \\
        \bottomrule
    \end{tabular}
\end{table}

\section{Features Relacionadas a Propriedades Atômicas}

Essas \textit{features} são derivadas das propriedades dos elementos químicos que compõem o material, calculadas como estatísticas sobre a fórmula química.

\begin{table}[H]
    \centering
    \caption{Descrição das \textit{features} relacionadas a propriedades atômicas.}
    \begin{tabular}{lp{10cm}}
        \toprule
        \textbf{Feature} & \textbf{Descrição Física} \\
        \midrule
        \texttt{atomic\_number\_mean} & Número atômico médio dos elementos. Relacionado à massa e ao número de elétrons. \\
        \texttt{electronegativity\_mean} & Eletronegatividade média. Afeta a distribuição de carga e a natureza das ligações químicas. \\
        \texttt{atomic\_radius\_mean} & Raio atômico médio. Relacionado ao volume da célula unitária e à densidade do material. \\
        \texttt{valence\_electrons\_mean} & Número médio de elétrons de valência. Determina a química de ligação e a estrutura eletrônica. \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{thebibliography}{99}
    \bibitem{onnes1911} H. K. Onnes, "The resistance of pure mercury at helium temperatures," \textit{Comm. Phys. Lab. Univ. Leiden}, vol. 12, pp. 122b, 124c, 1911.
    \bibitem{bcs1957} J. Bardeen, L. N. Cooper, and J. R. Schrieffer, "Theory of Superconductivity," \textit{Physical Review}, vol. 108, no. 5, pp. 1175–1204, 1957.
    \bibitem{bednorz1986} J. G. Bednorz and K. A. Müller, "Possible high $T_c$ superconductivity in the Ba-La-Cu-O system," \textit{Zeitschrift für Physik B Condensed Matter}, vol. 64, no. 2, pp. 189–193, 1986.
    \bibitem{kamihara2008} Y. Kamihara et al., "Iron-Based Layered Superconductor La[O$_{1-x}$F$_x$]FeAs (x = 0.05--0.12) with $T_c$ = 26 K," \textit{Journal of the American Chemical Society}, vol. 130, no. 11, pp. 3296–3297, 2008.
    \bibitem{stanev2018} V. Stanev et al., "Machine learning modeling of superconducting critical temperature," \textit{npj Computational Materials}, vol. 4, no. 1, p. 29, 2018.
    \bibitem{himanen2019} L. Himanen et al., "DScribe: Library of descriptors for machine learning in materials science," \textit{Advanced Science}, vol. 7, no. 3, p. 1902410, 2020.
    \bibitem{hamidieh2018} K. Hamidieh, "A data-driven statistical model for predicting the critical temperature of a superconductor," \textit{Computational Materials Science}, vol. 154, pp. 346-354, 2018.
    \bibitem{london1935} F. London and H. London, "The Electromagnetic Equations of the Supraconductor," \textit{Proceedings of the Royal Society of London A}, vol. 149, no. 866, pp. 71-88, 1935.
    \bibitem{ginzburg1950} V. L. Ginzburg and L. D. Landau, "On the theory of superconductivity," \textit{Zh. Eksp. Teor. Fiz.}, vol. 20, pp. 1064-1082, 1950.
    \bibitem{tibshirani1996} R. Tibshirani, "Regression Shrinkage and Selection via the Lasso," \textit{Journal of the Royal Statistical Society. Series B}, vol. 58, no. 1, pp. 267–288, 1996.
    ...
    \bibitem{chen2016} T. Chen and C. Guestrin, "XGBoost: A Scalable Tree Boosting System," in \textit{Proceedings of the 22nd ACM SIGKDD}, 2016, pp. 785–794.
    \bibitem{ke2017} G. Ke et al., "LightGBM: A Highly Efficient Gradient Boosting Decision Tree," in \textit{Advances in Neural Information Processing Systems 30}, 2017, pp. 3146–3154.
    \bibitem{supercon} National Institute for Materials Science (NIMS), "SuperCon: Database for Superconducting Materials," Japan. [Online]. Available: https://supercon.nims.go.jp/
    \bibitem{delucca2026} L. M. De Lucca, "Classifica\c{c}\~ao de Supercondutores Utilizando T\'ecnicas de Aprendizado de M\'aquina," Trabalho da disciplina de F\'isica Computacional, UFABC, 2025.
    \bibitem{scidb2024} Chinese Academy of Sciences, "SciDB: Scientific Data Bank," [Online]. Available: https://www.scidb.cn/
    \bibitem{ouyang2018} R. Ouyang, S. Curtarolo, E. Ahmetcik, M. Scheffler, and L. M. Ghiringhelli, "SISSO: A compressed-sensing method for identifying the best low-dimensional descriptor in an immensity of offered candidates," \textit{Physical Review Materials}, vol. 2, no. 8, p. 083802, 2018.
    ...
    \bibitem{drozdov2015} A. P. Drozdov et al., "Conventional superconductivity at 203 kelvin at high pressures in the sulfur hydride system," \textit{Nature}, vol. 525, pp. 73--76, 2015.
\end{thebibliography}
