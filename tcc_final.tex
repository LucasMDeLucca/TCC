...
\begin{titlepage}
    ...
    % TODO_DEFESA: substituir o placeholder abaixo pelo nome real do orientador antes da entrega final
    {\large Orientador: Prof.\ Dr.\ \textbf{[Nome do Orientador --- PREENCHER ANTES DA DEFESA]}}
    \vfill
...
O dataset final (\texttt{data\_treino.csv}) contém 2.474 amostras de materiais, cada uma descrita por 116 \textit{features} numéricas candidatas (após exclusão dos identificadores \texttt{id}, \texttt{group\_id}, da variável alvo $T_c$ e de 12 colunas string \texttt{atoms\_0}\ldots\texttt{atoms\_11} que codificam apenas a composição elementar e são descartadas pelo \texttt{select\_dtypes} no pipeline) e uma variável alvo binária derivada de $T_c$. Após o pré-processamento descrito na seção seguinte (remoção de colunas com mais de 50\% de valores ausentes e \textit{features} com variância zero), o conjunto final utilizado pelos modelos foi reduzido a 53 \textit{features}.
